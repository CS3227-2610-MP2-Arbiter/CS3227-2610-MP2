...
- The portable shape made case handling explicit. A case probe against the found file was written, passed on macOS, and failed on the Windows CI job twice: `Path.equals` ignores case there, and the mismatch message then used the platform separator. Both were fixable, but the check cannot be portable, because whether a name resolves regardless of case is a property of the file system, not of the path. It was removed, and case-exactness is now recorded as a naming convention that registration ([#25]) enforces. The Windows job caught a real problem rather than confirming a fix.
...
- After the second review round: JDK 25 `./gradlew cleanTest check shadowJar --no-daemon` passed with 137 JUnit tests, none skipped, both Checkstyle tasks, and the release jar; 134 after the case probe was removed, with none skipped.
