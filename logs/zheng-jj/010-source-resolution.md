...
- The portable shape made case handling explicit, and the resolver now checks the recorded case against the file it found. The check only fires when the two paths are otherwise identical, so an inside-media link, whose target is a different path, is not mistaken for a case mismatch. Tests cover a wrong-case directory and file, and both ran on this machine's case-insensitive filesystem rather than being skipped.
...
- After the second review round: JDK 25 `./gradlew cleanTest check shadowJar --no-daemon` passed with 137 JUnit tests, none skipped, both Checkstyle tasks, and the release jar.
