...
4. Write JUnit tests under `src/test/java` with focused assertions on the specific expected result or exception. Name each test method `featureUnderTest_testScenario_expectedBehavior` (e.g., `sortList_emptyList_exceptionThrown` or `getMember_memberNotFound_nullReturned`). Omit the expected behavior when the test covers every variation of the scenario (e.g., `sortList_emptyList`), and omit both when it covers every scenario (e.g., `sortList`). Use the test types in the plan; without one, prefer unit tests for isolated logic and integration tests for role isolation, shared services and persistence. Use synthetic fixtures and temporary storage. Leave production code unchanged.
