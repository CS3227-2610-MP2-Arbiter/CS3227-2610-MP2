...
- **`ClassificationWorkflow`** seeds one project, its taxonomy, a registered corpus, one split, accounts, assignments, submissions and resolutions in one committed action:
  ...
  - `majority`, `adjudicated` and `mean` record a resolution the test names, so the expected answer is visible in the test. The fixture checks it against the answers under rule 10: a majority label must hold a strict majority, an adjudicated item must have none, and a mean must be the ratings' exact mean. It never picks a resolution itself.
...
- **`BlindnessRule`** imports the compiled classes with ArchUnit and walks method by method from every annotator-facing method, through method and constructor calls, method references and lambda bodies, into service and UI code. A call through an interface or overridable method also follows its implementations. Constructing an application class also reaches its methods that override a JDK or JavaFX method, such as `Task.call`, because the framework rather than the caller runs them; its other methods stay unreached, so constructing a service does not reach its adjudicator-only reads. It stops at repository interfaces, the model and the workspace layer, and reports the path to each violation:
  ...
  - **Resolved results:** any `ResolutionRepository` method, and any use of a resolution type in a reached method: in its signature, a call, a constructor or a field access. So a resolution handed over untyped, as `Node.getUserData()` returns one, is caught when it is read.
...
- Teammate review on PR #69 found three gaps, now closed. A resolution was only caught through a repository call or a signature, so direct use of the type now counts. Framework callbacks such as `Task.call` were not followed, so constructing an application class now reaches its JDK or JavaFX overrides. The fixture only counted a resolution's answers, so it now checks the resolution against them. Removing each change temporarily made its new tests fail.
- JDK 25 `./gradlew cleanTest check shadowJar --no-daemon` passed: 185 JUnit tests (51 new), none skipped, both Checkstyle tasks, and the release jar.
