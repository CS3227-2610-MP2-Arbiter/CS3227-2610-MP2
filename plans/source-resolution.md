...
- **Containment is decided twice, on path elements rather than a string prefix.** The stored path is joined to the workspace root and normalised, and the result must be the media root itself or below it, so `media-lookalike/` and `../` are refused before the disk is touched. Because only a real path shows where a link points, the same check runs again on `Path.toRealPath()` afterwards; that is the check a symlink or junction cannot pass.
...
| Traversal, prefix-lookalike paths and links/junctions cannot expose content outside the resolved media root | Unit tests: `media/../outside.txt`, a sibling `media-lookalike/` file and a symlink to an outside file all fail; a symlink staying inside `media/` resolves. Windows-only cases for a backslash traversal, a UNC path and a drive-absolute path run on the Windows CI job |
...
- JDK 25 `./gradlew cleanTest check shadowJar --no-daemon` passed: 113 JUnit tests, both Checkstyle tasks, and the release jar. Three Windows-only cases skip on macOS and run on the Windows CI job.
