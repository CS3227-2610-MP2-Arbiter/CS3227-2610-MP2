...
- **The stored case is a naming convention, not a check.** A stored path is used exactly as written and is never folded, so registration ([#25]) is what keeps the stored name and the file's name the same. Whether a name resolves regardless of case is a property of the file system, not of the path: a probe for it cannot be portable, and one attempt at it passed on macOS but failed on the Windows CI job.
...
- **Who enforces case-exactness?** The resolver treats a stored path exactly as written but does not probe the file system for it, because the answer depends on the file system rather than the path. If the owner wants registration to refuse a stored name whose case differs from the file's, that check belongs in [#25], where the name is read from the directory listing.
