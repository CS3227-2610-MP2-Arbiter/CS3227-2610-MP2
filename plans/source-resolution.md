...
- **Reads do not check the stored case.** A stored path is used exactly as written and is never folded. Whether a name resolves regardless of case is a property of the file system, not of the path: a probe for it cannot be portable, and one attempt at it passed on macOS but failed on the Windows CI job. Registration ([#25]) later made `resolveForImport` refuse a chosen name spelled differently from the one on disk.
