...
- **A stored path has one portable shape, checked before the disk is touched.** It is relative, uses `/` between segments, starts with `media/`, and contains no empty, `.` or `..` segment. A `\` or `:` is refused outright, because both are separators or drive syntax on Windows.
- **The recorded case is checked against the file that was found.** Case is never folded, so a path that resolves only because the filesystem ignores case is refused there rather than working on one computer and failing on another. The comparison runs only when the recorded path and the found path are otherwise the same, so a link recorded inside `media/` is not mistaken for a case mismatch; it also cannot run before the disk is read, since the found name is what it compares against.
...
- JDK 25 `./gradlew cleanTest check shadowJar --no-daemon` passed: 137 JUnit tests, none skipped, both Checkstyle tasks, and the release jar.
