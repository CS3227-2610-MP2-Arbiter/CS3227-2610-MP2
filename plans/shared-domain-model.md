...
**Status:** Initial models/interfaces implemented, awaiting human review; accepted detection, lifecycle and provenance follow-ups below remain unimplemented. The issue is co-owned by zheng-jj and Whimsyturtle and says to agree this "together, on one screen", so the decisions below still need both owners' sign-off.
...
- **Freeze setup after assignment and seal completed projects.** Whimsyturtle accepted [User Flows rules 3/6/9/13/14](../docs/UserFlows.md#3-rules-both-tracks-share): permanently freeze corpus/taxonomy at the project's first assignment; lock each assigned split's definition, including membership/order, *k* and reward; make COMPLETE seal all project records and earnings inputs, preventing reopen or destructive project deletion. Pre-completion work/returns and flag review continue, with flagged exclusion retaining item/membership/answer/provenance records. Never-assigned splits may still be configured from the frozen corpus before completion. Removing current assignments does not undo first-assignment locks.
- **Lifecycle implementation follow-up.** Preserve first-assignment evidence and enforce locks/sealing transactionally in services/persistence. Current mutable POJOs and repository interfaces do not provide those guarantees by themselves. Update the stale `SplitItem`/`Split` membership and `LabelRepository` deletion contracts during model fixes; label deletion must no longer cascade into annotations. Account replacement and the exact repair/resubmission flow remain pending decisions. User Flows rules 17/18 settle the current-evidence scope and require atomic invalidation; implementation is still pending. Source-file integrity for by-reference imports remains a separate enforcement concern. This requirements change adds no new state enum, ledger or revision system.
- **Keep latest submitted evidence and the current decision.** Whimsyturtle chose [User Flows rules 17/18](../docs/UserFlows.md#3-rules-both-tracks-share): retain each annotator's latest submitted snapshot, including unselected answers and withdrawn snapshots marked ineligible; the current resolution/contributors or selected box set/method/decider/time; current flags/dispositions; and latest return reason/person/time. Defer earlier answer/draft revisions, previous decisions, repeated return/flag histories and setup-edit timelines. During rework, retain the latest submission separately from the working draft; valid resubmission replaces it. COMPLETE seals the retained records.
- **Provenance implementation follow-up.** The current single mutable `Annotation` plus `submitted` flag does not yet preserve a last-submitted snapshot alongside an edited draft. Agree that bounded representation without a full revision archive, the current-contributor contract, current repair attribution, and an operation that completely invalidates a current `Resolution`. Withdrawal/correction and invalidation must be atomic; changed detection boxes require fresh selection even if the annotation ID is reused. Existing latest-return fields fit the reduced history scope. No source implementation is added by this decision.
...
- **`Split` gained `SplitItem`, `SplitStrategy` and `seed`.** The reviewer spotted that a split had no way to record which items it held. Membership is now its own record; the later accepted lifecycle decision limits membership edits to never-assigned splits. Flag exclusion preserves those records. The seed is stored because rule 7 requires a seeded split to be reproducible.
