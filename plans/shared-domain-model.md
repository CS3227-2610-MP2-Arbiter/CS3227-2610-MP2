...
**Status:** Initial models/interfaces implemented, awaiting human review; accepted detection, lifecycle, provenance, count-only allocation, forward-only submission, fixed-assignment, money-removal and statistics-contract follow-ups remain unimplemented. The issue is co-owned by zheng-jj and Whimsyturtle; this plan records accepted scope alongside the remaining model cleanup.
...
- **Lifecycle implementation follow-up.** Persist first-assignment evidence and enforce locks/sealing transactionally in services/persistence. Mutable POJOs and repository interfaces do not supply those guarantees. Update stale membership/deletion contracts; label deletion cannot cascade into answers. Account replacement is deferred by the later fixed-assignment decision; by-reference source integrity remains a follow-up.
...
- **Money tracking is deferred beyond v1.0.0.** The earlier [#2](https://github.com/CS3227-2610-MP2-Arbiter/CS3227-2610-MP2/issues/2) vesting decision is historical and superseded for v1. Remove per-item reward configuration, pending/released earnings, the completed-assignment pay gate, the report-only pay question and the income CSV from the v1 contracts; [#20](https://github.com/CS3227-2610-MP2-Arbiter/CS3227-2610-MP2/issues/20) and [#21](https://github.com/CS3227-2610-MP2-Arbiter/CS3227-2610-MP2/issues/21) are deferred. Progress and lifetime valid-annotation counts remain. **Contract cleanup remains:** remove obsolete `Split.itemReward` and earnings-specific model/repository comments or contracts, while preserving the valid-annotation count needed by [#19](https://github.com/CS3227-2610-MP2-Arbiter/CS3227-2610-MP2/issues/19). Current Java source is unchanged by this documentation decision.
...
- **`countSubmittedByAnnotator` remains useful only for annotation totals.** Its earnings-specific comment is obsolete now that money tracking is deferred. Cleanup must define/count valid submitted annotations for [#19](https://github.com/CS3227-2610-MP2-Arbiter/CS3227-2610-MP2/issues/19), excluding drafts and report-only outcomes; this documentation decision does not change the repository source.
