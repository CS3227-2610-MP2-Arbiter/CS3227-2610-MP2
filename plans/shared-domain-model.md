...
3. **A `SCALE` answer is stored as a whole number.** `Annotation.scaleValue` is an `Integer`; the range lives on `TaxonomySettings`, not `Project`. Only the value agreed at resolution is a `Double`, since it may be an average. Superseded in part by the review rounds below.
