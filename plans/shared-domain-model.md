...
**Status:** Initial models/interfaces implemented, awaiting human review; accepted detection and lifecycle follow-ups below remain unimplemented. The issue is co-owned by zheng-jj and Whimsyturtle and says to agree this "together, on one screen", so the decisions below still need both owners' sign-off.
...
- **Lifecycle implementation follow-up.** Preserve first-assignment evidence and enforce locks/sealing transactionally in services/persistence. Current mutable POJOs and repository interfaces do not provide those guarantees by themselves. Update the stale `SplitItem`/`Split` membership and `LabelRepository` deletion contracts during model fixes; label deletion must no longer cascade into annotations. Account replacement, return/repair resolution invalidation and historical revision scope remain pending decisions. Source-file integrity for by-reference imports remains a separate enforcement concern. This requirements change adds no new state enum, ledger or revision system.
