...
**Status:** Initial models/interfaces implemented, awaiting human review; accepted detection, lifecycle, provenance, count-only allocation, forward-only submission and fixed-assignment follow-ups remain unimplemented. The issue is co-owned by zheng-jj and Whimsyturtle; this plan records accepted scope alongside the remaining model cleanup.
