...
16. **Detection resolves by selecting one complete submission.** After *k* valid submitted annotations, an adjudicator selects exactly one annotator's complete box set for that item, preserving every box's coordinates and label. There is no automatic box matching, mixing of submissions or box editing during detection resolution, even when submissions appear identical. Until a set is selected, the item remains unresolved. Record the selected annotation, method, decider and timestamp, and retain the other annotators' latest submitted snapshots for provenance (rule 17). Export uses exactly the currently selected eligible set. [#34], [#36], [#37]
17. **Provenance keeps latest submissions and the current decision.** Retain each annotator's latest submitted answer, including unselected answers, with attribution and timestamps; the current valid resolution with its contributors or selected detection set, method, decider and time; current flags/dispositions; and the latest return reason, person and time. During rework, retain the latest submitted snapshot as withdrawn/ineligible and autosave the working draft separately. Valid resubmission replaces that snapshot; older revisions need not be kept. Earlier answer/draft versions, previous resolutions, repeated return/flag timelines and setup-edit timelines are deferred. COMPLETE seals the retained current records. [#17], [#36], [#37]
18. **Changed inputs invalidate the current decision.** Before COMPLETE, returning work withdraws its submitted inputs and invalidates affected resolutions atomically, before editing begins. Drafts and withdrawn submissions cannot count toward *k*, current decision contributors or final exported answers. Corrections of submitted content must also invalidate dependent decisions atomically and preserve truthful authorship. After valid resubmission, re-evaluate the existing classification rules; manual cases, including detection, require a fresh decision. Reusing an annotation ID must never preserve a stale box selection. No historical copy of the invalidated decision is required. [#17], [#27], [#34], [#35]
