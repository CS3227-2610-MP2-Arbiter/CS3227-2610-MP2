...
2. **Persist completed actions.** Each completed logical action commits immediately. In particular, **Submit & next** stores an immutable answer and advances the queue in one transaction; an unsubmitted choice is screen state, not a database record. [#6], [#17]
...
14. **Each split's definition locks on its first assignment.** Its membership/order, *k* and other setup settings cannot change afterwards. Before then, changing membership means deleting the batch and generating a replacement; there is no manual item selection or movement. [#28], [#32]
