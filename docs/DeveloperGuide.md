...
- **Keep current evidence.** Retain each annotator's latest submitted answer, including unselected answers, and the current decision with its contributors and metadata. Current flags/dispositions and latest return metadata remain available; earlier submission/decision versions and edit timelines are deferred. During rework, preserve the withdrawn submission separately from the autosaved draft until a valid resubmission replaces it.
- **Invalidate before correction.** Returning or correcting submitted input must invalidate affected decisions atomically. Only eligible submitted inputs feed a new majority/mean or manual choice; an edited detection answer never inherits its old selection merely because its ID is unchanged. The [shared rules](UserFlows.md#3-rules-both-tracks-share) define this required behavior; its persistence/service implementation remains pending.
